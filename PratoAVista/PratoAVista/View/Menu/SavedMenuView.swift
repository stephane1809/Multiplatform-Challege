...
struct SavedMenuView: View {
    ...
    var restaurantRecordName: String
    ...
    func saveAction() {
        if imageOnSaveButton == "bookmark.fill" {
            ...
            restaurantManager.removeFromSaved(recordName: restaurantRecordName)
        } else {
            imageOnSaveButton = "bookmark.fill"
            Task {
                await restaurantManager.saveRestaurant(recordName: restaurantRecordName)
            }
        }
